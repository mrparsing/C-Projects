#include <stdio.h>
#include <stdint.h>

void subBytes(uint8_t state[4][4], uint8_t *sbox)
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = sbox[state[i][j]];
}

void rotateLeft(uint8_t *row, int n)
{
    for (int r = 0; r < n; r++)
    {
        uint8_t tmp = row[0];
        row[0] = row[1];
        row[1] = row[2];
        row[2] = row[3];
        row[3] = tmp;
    }
}

void shiftRows(uint8_t state[4][4])
{
    rotateLeft(state[1], 1);
    rotateLeft(state[2], 2);
    rotateLeft(state[3], 3);
}

uint8_t gmult(uint8_t a, uint8_t b)
{
    uint8_t p = 0;
    for (int i = 0; i < 8; i++)
    {
        if (b & 1)
            p ^= a;

        uint8_t hi_bit_set = a & 0x80;
        a <<= 1;
        if (hi_bit_set)
            a ^= 0x1b;

        b >>= 1;
    }
    return p;
}

void mixColumns(uint8_t state[4][4])
{
    for (int j = 0; j < 4; j++)
    {
        uint8_t s0 = state[0][j];
        uint8_t s1 = state[1][j];
        uint8_t s2 = state[2][j];
        uint8_t s3 = state[3][j];

        state[0][j] = gmult(0x02, s0) ^ gmult(0x03, s1) ^ s2 ^ s3;
        state[1][j] = s0 ^ gmult(0x02, s1) ^ gmult(0x03, s2) ^ s3;
        state[2][j] = s0 ^ s1 ^ gmult(0x02, s2) ^ gmult(0x03, s3);
        state[3][j] = gmult(0x03, s0) ^ s1 ^ s2 ^ gmult(0x02, s3);
    }
}

void addRoundKey(uint8_t state[4][4], uint8_t *roundKey)
{
    int k = 0;
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++)
            state[i][j] ^= roundKey[k++];
}

uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

void keyExpansion(uint8_t *key, uint8_t *expandedKeys, uint8_t *sbox)
{
    for (int i = 0; i < 16; i++)
        expandedKeys[i] = key[i];

    int bytesGenerated = 16;
    int rconIteration = 1;
    uint8_t t[4];

    while (bytesGenerated < 176)
    {
        for (int k = 0; k < 4; k++)
            t[k] = expandedKeys[bytesGenerated - 4 + k];

        if (bytesGenerated % 16 == 0)
        {
            // RotWord
            uint8_t tmp = t[0];
            t[0] = t[1];
            t[1] = t[2];
            t[2] = t[3];
            t[3] = tmp;

            // SubWord
            for (int k = 0; k < 4; k++)
                t[k] = sbox[t[k]];

            // XOR con Rcon
            t[0] ^= rcon[rconIteration++];
        }

        for (int k = 0; k < 4; k++)
        {
            expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - 16] ^ t[k];
            bytesGenerated++;
        }
    }
}

void rotateRight(uint8_t *row, int n)
{
    for (int r = 0; r < n; r++)
    {
        uint8_t tmp = row[3];
        row[3] = row[2];
        row[2] = row[1];
        row[1] = row[0];
        row[0] = tmp;
    }
}

void invShiftRows(uint8_t state[4][4])
{
    rotateRight(state[1], 1);
    rotateRight(state[2], 2);
    rotateRight(state[3], 3);
}

uint8_t inv_sbox[256] = {
    };

void invSubBytes(uint8_t state[4][4], uint8_t *inv_sbox)
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] = inv_sbox[state[i][j]];
}

void invMixColumns(uint8_t state[4][4])
{
    for (int j = 0; j < 4; j++)
    {
        uint8_t s0 = state[0][j];
        uint8_t s1 = state[1][j];
        uint8_t s2 = state[2][j];
        uint8_t s3 = state[3][j];

        state[0][j] = gmult(0x0e, s0) ^ gmult(0x0b, s1) ^ gmult(0x0d, s2) ^ gmult(0x09, s3);
        state[1][j] = gmult(0x09, s0) ^ gmult(0x0e, s1) ^ gmult(0x0b, s2) ^ gmult(0x0d, s3);
        state[2][j] = gmult(0x0d, s0) ^ gmult(0x09, s1) ^ gmult(0x0e, s2) ^ gmult(0x0b, s3);
        state[3][j] = gmult(0x0b, s0) ^ gmult(0x0d, s1) ^ gmult(0x09, s2) ^ gmult(0x0e, s3);
    }
}

int main()
{
    uint8_t sbox[256] = {
        };

    uint8_t input[16] = "asdfghjklzxcvbnm";
    uint8_t key[16] = "asdfghjklzxcvbnm";

    uint8_t expandedKeys[176];
    keyExpansion(key, expandedKeys, sbox);

    uint8_t state[4][4];
    int k = 0;
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++)
            state[i][j] = input[k++];

    addRoundKey(state, &expandedKeys[0]);

    for (int round = 1; round <= 9; round++)
    {
        subBytes(state, sbox);
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, &expandedKeys[round * 16]);
    }

    subBytes(state, sbox);
    shiftRows(state);
    addRoundKey(state, &expandedKeys[160]);

    printf("\nCiphertext:\n");
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++)
            printf("%c", state[i][j]);
    printf("\n");

    addRoundKey(state, &expandedKeys[160]);

    for (int round = 9; round >= 1; round--)
    {
        invShiftRows(state);
        invSubBytes(state, inv_sbox);
        addRoundKey(state, &expandedKeys[round * 16]);
        invMixColumns(state);
    }

    invShiftRows(state);
    invSubBytes(state, inv_sbox);
    addRoundKey(state, &expandedKeys[0]);

    printf("\nPlaintext:\n");
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++)
            printf("%c", state[i][j]);
    printf("\n");
    return 0;
}